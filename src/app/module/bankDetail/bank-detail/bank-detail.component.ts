...
export class BankDetailComponent {
  ...
  constructor(private formBuilder: FormBuilder, private sharedData:SharedDataService, private services: ApiDataService, 
    private globalService: GlobalService, private router: Router, private toastrService: ToastrService){
    ...
    ngOnInit(): void {
      ...
      this.bankdetform = this.formBuilder.group({
        ...
        mobNo:['', [Validators.required, Validators.maxLength(10), Validators.min(10), Validators.pattern('^[0-9\-\']+')]],
   ...
   (error:any) => (
    ...
    getResponse(val:any){
      if(val.result.active=="yes"){
        this.toastrService.success('Your Bank Details have been verified.', 'Congrats!')
